...
macro( XSD_SCHEMA NAME FILE )
  ...
  set(oneValueArgs --hxx-suffix --cxx-suffix --output-dir)
  cmake_parse_arguments(_XSD_FILE_ARGS "" "${oneValueArgs}" "" ${ARGN})
  
  set(_XSD_CXX_SUFFIX ".cxx")
  set(_XSD_HXX_SUFFIX ".hxx")
  set(_XSD_OUTPUT_DIR "${CMAKE_CURRENT_BINARY_DIR}")

  if(DEFINED _XSD_FILE_ARGS_--cxx-suffix)
    set(_XSD_CXX_SUFFIX "${_XSD_FILE_ARGS_--cxx-suffix}")
  endif()
  if(DEFINED _XSD_FILE_ARGS_--hxx-suffix)
    set(_XSD_HXX_SUFFIX "${_XSD_FILE_ARGS_--hxx-suffix}")
  endif()
  if(DEFINED _XSD_FILE_ARGS_--output-dir)
    set(_XSD_OUTPUT_DIR "${_XSD_FILE_ARGS_--output-dir}")
  endif()

  get_filename_component( xs_FILE "${FILE}" NAME_WE )
  set(generated_files)
  set( ${NAME}_SOURCES "${_XSD_OUTPUT_DIR}/${xs_FILE}${_XSD_CXX_SUFFIX}" )
  list(APPEND generated_files ${${NAME}_SOURCES})
  set( ${NAME}_HEADERS "${_XSD_OUTPUT_DIR}/${xs_FILE}${_XSD_HXX_SUFFIX}" )
  list(APPEND generated_files ${${NAME}_HEADERS})

  list(FIND ARGN "--generate-inline" generate_inline)
  if(generate-inline)
    set( ${NAME}_INLINES "${_XSD_OUTPUT_DIR}/${xs_FILE}.ixx" )
