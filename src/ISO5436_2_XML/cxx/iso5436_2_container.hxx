...
namespace OpenGPS
{
	...
	private:
		 ...
		 */
		bool VerifyChecksum(const std::filesystem::path& filePath, const UnsignedBytePtr checksum, size_t size) const;
		 ...
		 * @returns Returns true when the checksum could be verified, false otherwise.
		 */
		bool VerifyChecksum(const std::filesystem::path& filePath, std::array<UnsignedByte, 16>& checksum) const;
		 ...
		 * @returns Returns true on success, false otherwise.
		 */
		bool ReadMd5FromFile(const std::filesystem::path& fileName, std::array<UnsignedByte, 16>& checksum) const;
